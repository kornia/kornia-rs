...
fn run_dense_parity(image: &str) {
    ...
    let weights = match load_packed_weights() {
        Some(w) => w,
        None => { eprintln!("[skip] packed weights absent or placeholder"); return; }
    };

    // MANIFEST for tolerances
    let manifest_text = std::fs::read_to_string(fixture_path("MANIFEST.toml"))
        .expect("MANIFEST.toml");
    let manifest: toml::Table = manifest_text.parse().unwrap();

    // Look up per-image height/width from [[images]] array
    let (h, w) = image_hw(&manifest, image)
        .unwrap_or_else(|| (manifest["input"]["height"].as_integer().unwrap() as usize,
    ...
    let input = match load_input(image, h, w) {
        Some(v) => v,
        None => { eprintln!("[skip] input not parseable for '{image}'"); return; }
    };

    let cfg = XFeatConfig { height: h, width: w, ..XFeatConfig::default() };
    let mut model = XFeat::new(cfg, weights).expect("construct").with_scalar_backend();
    ...
    for (&ours, &exp) in our_k1h.iter().zip(kp_expected.iter()) {
        ...
        if err > kp_max_err { kp_max_err = err; }
        if err > kp_abs { kp_fail += 1; }
    }
    for (&ours, &exp) in our_h1rel.iter().zip(rel_expected.iter()) {
        let err = (ours - exp).abs();
        if err > rel_max_err { rel_max_err = err; }
        if err > rel_abs { rel_fail += 1; }
    }

    assert_eq!(kp_fail, 0,
        "[{image}] k1h: {kp_fail} exceed abs tol {kp_abs:.1e}; max err = {kp_max_err:.2e}");
    assert_eq!(rel_fail, 0,
...
fn run_sparse_parity(image: &str) {
    ...
    let exp_kps: Vec<[f32; 2]> = json["keypoints"].as_array().unwrap()
        .iter()
        .map(|kp| {
    ...
    let exp_descs: Vec<Vec<f32>> = json["descriptors"].as_array().unwrap()
        .iter()
        .map(|d| d.as_array().unwrap().iter().map(|v| v.as_f64().unwrap() as f32).collect())
        .collect();

    // Count check
    let our_n = our_kps.len();
    let exp_n = exp_kps.len();
    assert!(our_n.abs_diff(exp_n) <= count_abs,
    ...
    for exp_kp in exp_kps.iter().take(n_check) {
        if our_kps.iter().any(|kp| (kp.x - exp_kp[0]).powi(2) + (kp.y - exp_kp[1]).powi(2) <= match_r2) {
            matched += 1;
        }
    }
    let union = (our_n + exp_n).saturating_sub(matched);
    let iou = if union == 0 { 1.0 } else { matched as f32 / union as f32 };
    assert!(iou >= iou_min,
    ...
    'outer: for (ei, exp_kp) in exp_kps.iter().enumerate().take(n_desc) {
        let Some(oi) = our_kps.iter().position(|kp| {
            (kp.x - exp_kp[0]).powi(2) + (kp.y - exp_kp[1]).powi(2) <= match_r2
        }) else { continue 'outer };
        ...
        let cos_dist = 1.0 - if na > 0.0 && nb > 0.0 { dot / (na * nb) } else { 0.0 };
        if cos_dist > cos_dist_max { cos_dist_max = cos_dist; }
        if cos_dist > cosdist_max  { desc_fail += 1; }
    }

    assert_eq!(desc_fail, 0,
...
#[test]
fn dense_parity_ref() { run_dense_parity("ref"); }

#[test]
fn dense_parity_tgt() { run_dense_parity("tgt"); }
...
fn run_dense_parity_root(dense_path: &std::path::Path) {
    let weights = match load_packed_weights() {
        Some(w) => w,
        None => { eprintln!("[skip] packed weights absent"); return; }
    };
    let manifest_text = std::fs::read_to_string(fixture_path("MANIFEST.toml")).unwrap();
    let manifest: toml::Table = manifest_text.parse().unwrap();
    ...
    let input = match load_input_root(h, w) {
        Some(v) => v,
        None => { eprintln!("[skip] root input not parseable"); return; }
    };
    let cfg = XFeatConfig { height: h, width: w, ..XFeatConfig::default() };
    let mut model = XFeat::new(cfg, weights).expect("construct").with_scalar_backend();
    model.extract(&input).expect("extract");
    ...
    for (&o, &e) in our_k1h.iter().zip(kp_expected.iter())  { let d=(o-e).abs(); if d>kp_max{kp_max=d;} if d>kp_abs{kp_fail+=1;} }
    for (&o, &e) in our_h1rel.iter().zip(rel_expected.iter()){ let d=(o-e).abs(); if d>rel_max{rel_max=d;} if d>rel_abs{rel_fail+=1;} }
    assert_eq!(kp_fail,0,"k1h: {kp_fail} exceed {kp_abs:.1e}; max={kp_max:.2e}");
    assert_eq!(rel_fail,0,"h1_rel: {rel_fail} exceed {rel_abs:.1e}; max={rel_max:.2e}");
...
#[test]
fn sparse_parity_ref() { run_sparse_parity("ref"); }

#[test]
fn sparse_parity_tgt() { run_sparse_parity("tgt"); }
...
fn run_sparse_parity_root(sparse_path: &std::path::Path) {
    ...
    let exp_kps: Vec<[f32; 2]> = json["keypoints"].as_array().unwrap()
        .iter()
        .map(|kp| { let a=kp.as_array().unwrap(); [a[0].as_f64().unwrap() as f32, a[1].as_f64().unwrap() as f32] })
        .collect();
    let exp_descs: Vec<Vec<f32>> = json["descriptors"].as_array().unwrap()
        .iter()
        .map(|d| d.as_array().unwrap().iter().map(|v| v.as_f64().unwrap() as f32).collect())
    ...
    let our_n = our_kps.len();
    let exp_n = exp_kps.len();
    assert!(our_n.abs_diff(exp_n) <= count_abs, "count: ours={our_n} exp={exp_n}");

    let match_r2 = 1.0f32;
    let n_check = our_n.min(exp_n);
    let mut matched = 0;
    for exp_kp in exp_kps.iter().take(n_check) {
        if our_kps.iter().any(|kp| (kp.x-exp_kp[0]).powi(2)+(kp.y-exp_kp[1]).powi(2) <= match_r2) { matched += 1; }
    }
    let union = (our_n + exp_n).saturating_sub(matched);
    let iou = if union == 0 { 1.0 } else { matched as f32 / union as f32 };
    ...
    let mut desc_fail = 0;
    'outer: for (ei, exp_kp) in exp_kps.iter().enumerate().take(n_desc) {
        let Some(oi) = our_kps.iter().position(|kp| (kp.x-exp_kp[0]).powi(2)+(kp.y-exp_kp[1]).powi(2) <= match_r2) else { continue 'outer };
        let our_d = &our_descs[oi*64..(oi+1)*64];
        let exp_d = &exp_descs[ei];
        let dot: f32 = our_d.iter().zip(exp_d).map(|(a,b)| a*b).sum();
        let na = our_d.iter().map(|v| v*v).sum::<f32>().sqrt();
        let nb = exp_d.iter().map(|v| v*v).sum::<f32>().sqrt();
        let cd = 1.0 - if na>0.0&&nb>0.0 {dot/(na*nb)} else {0.0};
        if cd > cos_dist_max { cos_dist_max = cd; }
        if cd > cosdist_max  { desc_fail += 1; }
    }
    assert_eq!(desc_fail, 0, "desc: {desc_fail} pairs exceed {cosdist_max:.1e}; max={cos_dist_max:.2e}");
