...
pub fn build_tracks(edges: &[TrackEdge]) -> Vec<FeatureTrack> {
    ...
    // 66% two-view points, against COLMAP's median 5 with 2.6%, on identical frames. It also gets
    // worse with feature density, which is why it bites us at 8192 keypoints per frame and not COLMAP
    // at ~2000.
    //
    // So: siblings are merged (keep the first -- they are the same pixel), a real disagreement still
    // drops the track. The counters are returned because this loss was previously invisible: the
    // `raw_tracks` figure the caller logs is already post-drop, and nothing counted what went.
...
mod tests {
    ...
    fn orientation_siblings_do_not_destroy_a_track() {
        ...
        let edges = vec![
            TrackEdge { cam_a: 0, kpt_a: 0, uv_a: p0, cam_b: 1, kpt_b: 10, uv_b: p1 },
            TrackEdge { cam_a: 1, kpt_a: 11, uv_a: p1, cam_b: 2, kpt_b: 0, uv_b: p2 },
            TrackEdge { cam_a: 0, kpt_a: 0, uv_a: p0, cam_b: 2, kpt_b: 0, uv_b: p2 },
        ];
        let tracks = build_tracks(&edges);
        assert_eq!(tracks.len(), 1, "the sibling collision must not delete the track");
    ...
    fn distinct_same_camera_observations_still_drop_the_track() {
        ...
        let p2 = Vec2F64::new(200.0, 300.0);
        let edges = vec![
            TrackEdge { cam_a: 0, kpt_a: 0, uv_a: p0, cam_b: 1, kpt_b: 10, uv_b: a },
            TrackEdge { cam_a: 1, kpt_a: 11, uv_a: b, cam_b: 2, kpt_b: 0, uv_b: p2 },
            TrackEdge { cam_a: 0, kpt_a: 0, uv_a: p0, cam_b: 2, kpt_b: 0, uv_b: p2 },
        ];
        assert!(build_tracks(&edges).is_empty(), "a real same-camera ambiguity must still drop");
    }
}
