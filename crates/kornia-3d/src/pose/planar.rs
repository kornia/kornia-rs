...
//!
//! # The square assumption, and where it does and does not bind
//!
//! The initial estimate comes from a homography fitted between a CANONICAL unit square
//! (`+/-1` in the order given) and the image points, so the initialisation is exact only when the
//! object points really are a square in that order. The orthogonal iteration that follows refines
//! against the caller's actual `object_pts`, so a non-square coplanar quad still converges -- it
//! simply starts further away. Callers with strongly non-square targets should raise `n_iters`.
//!
//! # Two solutions
//!
//! Planar pose is genuinely ambiguous: a plane viewed from one side admits a second pose that
//! reprojects almost as well, and the ambiguity is worst at small tilt and long range. Both are
//! returned ([`PlanarPosePair`]) rather than silently picking one, because a caller with extra
//! information (a second view, a motion prior, a known facing direction) can resolve it and a
//! caller without it needs to know the choice was made.
