...
impl std::ops::Mul<Vec2F32> for SO2F32 {
    type Output = Vec2F32;
...
impl std::ops::Mul<SO2F32> for SO2F32 {
    type Output = SO2F32;

    fn mul(self, other: SO2F32) -> Self::Output {
...
impl std::ops::MulAssign<SO2F32> for SO2F32 {
    #[inline]
...
mod tests {
    ...
    fn make_random_so2() -> SO2F32 {
        SO2F32::from_random()
    }

    #[test]
    fn test_identity() {
        let identity = SO2F32::IDENTITY;
    ...
    fn test_from_matrix() {
        ...
        let so2 = SO2F32::from_matrix(&mat);
    ...
    fn test_lplus_lminus_consistency() {
        ...
        let y = SO2F32::lplus(dtheta, &x); // τ ⊕ X → Y
        let diff = SO2F32::lminus(&y, &x); // Y ⊖ X → τ
        assert_relative_eq!(diff, dtheta, epsilon = EPSILON);
    }

    #[test]
    fn test_matrix() {
        let so2 = SO2F32::new(Vec2F32::new(0.6, 0.8));
    ...
    fn test_inverse() {
        ...
        let inv = so2.inverse();
        let result = so2 * inv;
        assert_relative_eq!(result.z.x, SO2F32::IDENTITY.z.x, epsilon = EPSILON);
        assert_relative_eq!(result.z.y, SO2F32::IDENTITY.z.y, epsilon = EPSILON);
    }

    #[test]
    fn test_adjoint() {
        let so2_id = SO2F32::IDENTITY;
        assert_relative_eq!(so2_id.adjoint(), 1.0f32);
        let so2_rand = SO2F32::from_random();
    ...
    fn test_exp() {
        ...
        // Test exp(0) = identity
        let so2_identity = SO2F32::exp(0.0);
    ...
    fn test_log() {
        ...
        // Test log(identity) = 0
        let identity_log = SO2F32::IDENTITY.log();
    ...
    fn test_hat() {
        ...
        // Test hat structure
        let hat_matrix = SO2F32::hat(theta);
    ...
    fn test_hat_vee_roundtrip() {
        let theta = 0.5;
        let hat_matrix = SO2F32::hat(theta);
        let vee_theta = SO2F32::vee(hat_matrix);
        assert_relative_eq!(vee_theta, theta);

        // Test with different values
        let values = [0.0, 0.1, -0.3, 1.0, -2.5];
        for &val in &values {
            let hat = SO2F32::hat(val);
            let vee = SO2F32::vee(hat);
    ...
    #[test]
    fn test_mul_so2() {
        let s1 = SO2F32::IDENTITY;
        let s2 = SO2F32::new(Vec2F32::new(0.9, 0.1).normalize());
    ...
    #[test]
    fn test_mul_assign() {
        let mut s = SO2F32::IDENTITY;
        let s2 = SO2F32::exp(0.5);
        s *= s2;
        assert_relative_eq!(s.z.x, s2.z.x);
        assert_relative_eq!(s.z.y, s2.z.y);

        let mut s3 = SO2F32::exp(0.3);
        let original_s3 = s3;
        let s4 = SO2F32::exp(0.2);
        s3 *= s4;
    ...
    fn test_from_matrix_matrix_roundtrip() {
        let so2 = make_random_so2();
        let matrix = so2.matrix();
        let so2_reconstructed = SO2F32::from_matrix(&matrix);
    ...
    fn test_specific_angles() {
        ...
        let so2_90 = SO2F32::exp(std::f32::consts::PI / 2.0);
        ...
        let so2_180 = SO2F32::exp(std::f32::consts::PI);
        ...
        let so2_270 = SO2F32::exp(3.0 * std::f32::consts::PI / 2.0);
    ...
    fn test_basic_verification() {
        ...
        let so2 = SO2F32::exp(std::f32::consts::PI / 4.0);

        // Test matrix roundtrip
        let matrix = so2.matrix();
        let so2_from_matrix = SO2F32::from_matrix(&matrix);
        ...
        let random_so2 = SO2F32::from_random();
