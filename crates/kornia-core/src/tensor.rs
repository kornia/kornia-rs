...
mod tests {
    ...
    fn get_3d() -> Result<(), TensorError> {
        ...
        assert_eq!(t.get([0, 1, 0]), Some(&4));
        assert_eq!(t.get([0, 1, 1]), Some(&5));
        assert_eq!(t.get([0, 1, 2]), Some(&6));
        assert_eq!(t.get([1, 0, 0]), Some(&7));
        assert_eq!(t.get([1, 0, 1]), Some(&8));
        assert_eq!(t.get([1, 0, 2]), Some(&9));
        assert_eq!(t.get([1, 1, 0]), Some(&10));
        assert_eq!(t.get([1, 1, 1]), Some(&11));
        assert_eq!(t.get([1, 1, 2]), Some(&12));
