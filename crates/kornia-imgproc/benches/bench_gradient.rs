...
use kornia_imgproc::filter::{
    kernels, spatial_gradient_float, spatial_gradient_float_by_separable_filter, spatial_gradient_float_parallel
};
...
fn bench_gradient(c: &mut Criterion) {
    ...
    for (width, height) in [(512, 512), (1024, 1024), (2048, 2048)].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("spatial_gradient_float_by_separable_filter", &parameter_string),
            &(&image, &output_dx, &output_dy),
            |b, i| {
                let (src, mut dx, mut dy) = (i.0, i.1.clone(), i.2.clone());
                b.iter(|| black_box(spatial_gradient_float_by_separable_filter(src, &mut dx, &mut dy)))
            },
        );
