...
macro_rules! tile_enumerator {
    ($doc: expr, $name:ident, $iterator:ident) => {
        #[doc = $doc]
        #[repr(transparent)]
        pub struct $name<'a, T>(pub $iterator<'a, T>);

        impl<'a, T> Iterator for $name<'a, T> {
            type Item = ((usize, usize), <$iterator<'a, T> as Iterator>::Item);
...
tile_enumerator!(
    "An enumerator over mutable tiles of an image, yielding the `(y, x)` tile indices and the tile data as mutable slices. \
    Each item is a tuple of `((tile_y, tile_x), tile)`, where tile is a mutable slice of rows, and each row is a mutable slice of pixel data.",
    TileEnumeratorMut,
    TileIteratorMut
);
...
/// An iterator over mutable tiles of an image, yielding non-overlapping rectangular regions as mutable slices.
///
/// Each item yielded by the iterator is a mutable slice of rows, where each row is a mutable slice of pixel data.
/// The tile size is specified at construction, and the iterator will yield tiles of the given size,
/// except for tiles at the image edges, which may be smaller if the image dimensions are not
/// multiples of the tile size.
pub struct TileIteratorMut<'a, T> {
    /// Reference to the image pixel data as a mutable flat slice.
    img_data: &'a mut [T],
    /// The width and height of the image.
    img_size: ImageSize,
    /// The size (width and height) of each tile in pixels.
    ...
    /// Index of the next tile to yield vertically.
    next_tile_y_index: usize,
    /// Buffer holding mutable references to the rows of the current tile.
    buffer: Vec<&'a mut [T]>,
}

impl<'a, T> TileIteratorMut<'a, T> {
    /// Creates a new `TileIteratorMut` from a mutable reference to an image and the desired tile size.
    ///
    /// # Arguments
    ///
    /// * `img` - A mutable reference to the image to be tiled.
    /// * `tile_size` - The width and height of each tile in pixels.
    ///
    /// # Returns
    ///
    /// Returns a `TileIteratorMut` that yields non-overlapping mutable tiles of the specified size from the image.
    /// Tiles at the image edges may be smaller if the image dimensions are not multiples of the tile size.
    pub fn from_image<const C: usize, A: ImageAllocator>(
        img: &'a mut Image<T, C, A>,
        tile_size: usize,
    ) -> Self {
        let img_size = img.size();
    ...
    /// and each row is a slice of pixel data.
    #[inline]
    pub fn tile_enumerator(self) -> TileEnumeratorMut<'a, T> {
        TileEnumeratorMut(self)
    }
}

impl<'a, T> Iterator for TileIteratorMut<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let img_data_ptr = self.img_data.as_mut_ptr();
        ...
        for y_px in 0..tile_y_px {
            ...
            let row_pxs =
                unsafe { std::slice::from_raw_parts_mut(img_data_ptr.add(start_index), tile_x_px) };
            self.buffer.push(row_pxs);
        }
        ...
        Some(unsafe { std::slice::from_raw_parts_mut(self.buffer.as_mut_ptr(), tile_y_px) })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_tile_iterator_mut() {
        let data = vec![127u8; 100];
        let mut image: Image<_, 1, _> = Image::new(
            ImageSize {
                width: 25,
                height: 4,
            },
        ...
        let tile_iter = TileIteratorMut::from_image(&mut image, 4);

        for tile in tile_iter {
            for tile_row in tile {
                let tile_row = tile_row as &mut [u8];
                for px in tile_row {
                    *px = 200;
                }
            }
        ...
        let expected = vec![200u8; 100];
        assert_eq!(image.as_slice(), expected.as_slice());
    }
}
