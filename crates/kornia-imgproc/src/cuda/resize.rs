...
//!
//! The `__ldg` kernels use no shared memory, so the SM's combined L1/smem
//! budget is fully allocated to the L1 data cache via `cuFuncSetCacheConfig`.
...
// ── CUDA C source: bilinear downscale, shared-memory tile ────────────────────
//
// For integer downscale factors ≥ 2 the block cooperatively loads a source tile
// of size (bw*scale_x+1) × (bh*scale_y+1) into `extern __shared__ float smem[]`
// with a single sequential sweep, then all threads read from smem without any
// further global memory accesses.  The sweep is coalesced (linear index stride
// through smem); the subsequent smem reads hit SRAM at ~2 cycles instead of
// ~30 for an L1 hit.
//
// Boundary clamping (BORDER_REPLICATE) is applied during tile loading — both
// out-of-bounds tile pixels and the bilinear right/bottom taps at the source
