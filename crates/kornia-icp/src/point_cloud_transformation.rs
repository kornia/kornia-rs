...
pub fn compute_centroids(
    ...
    let src_centroid = points_src.iter().fold(Vec3::ZERO, |acc, &p| acc + p) 
        / points_src.len() as f32;
    let dst_centroid = points_dst.iter().fold(Vec3::ZERO, |acc, &p| acc + p) 
        / points_dst.len() as f32;
...
pub fn fit_transformation(
    ...
    assert_eq!(points_src.len(), points_dst.len(), "Point clouds must have same number of points");
    assert!(points_src.len() >= 3, "Need at least 3 points for transformation estimation");
    ...
    if points_src.len() >= 4 {
        ...
        for i in 0..3 {
            ...
            match i {
                0 => { src_pts.x_axis = src_pt; dst_pts.x_axis = dst_pt; }
                1 => { src_pts.y_axis = src_pt; dst_pts.y_axis = dst_pt; }
                2 => { src_pts.z_axis = src_pt; dst_pts.z_axis = dst_pt; }
                _ => {}
            }
        }
...
mod tests {
    ...
    fn test_identity_transformation() {
        ...
        assert!(mat3_approx_eq(&r, &Mat3::IDENTITY, 1e-6), 
                "Expected identity rotation, got: {:?}", r);
        assert!(vec3_approx_eq(&t, &Vec3::ZERO, 1e-6),
                "Expected zero translation, got: {:?}", t);
    }
    ...
    fn test_pure_rotation() {
        ...
        assert!(mat3_approx_eq(&r, &expected_r, 1e-6),
                "Expected rotation:\n{:?}\nGot:\n{:?}", expected_r, r);
        assert!(vec3_approx_eq(&t, &Vec3::ZERO, 1e-6),
                "Expected zero translation, got: {:?}", t);
    }
    ...
    fn test_translation() {
        ...
        assert!(mat3_approx_eq(&r, &Mat3::IDENTITY, 1e-6),
                "Expected identity rotation, got: {:?}", r);
        assert!(vec3_approx_eq(&t, &translation, 1e-6),
                "Expected translation {:?}, got: {:?}", translation, t);
    }
