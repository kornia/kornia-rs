...
mod tests {
    ...
    fn test_find_closest_point() {
        ...
        // Position (5, 1) is closer to the start waypoint (0, 0) than to (10, 0)
        // Distance to (0,0): sqrt(25 + 1) = 5.1
        // Distance to (10,0): sqrt(25 + 1) = 5.1
        // Actually they're about the same, but (0,0) comes first
        let (idx, dist) = traj.find_closest_point(Vec2::new(5.0, 1.0)).unwrap();
        assert!(idx <= 1); // Could be 0 or 1
        assert!(dist > 0.0);
    }
