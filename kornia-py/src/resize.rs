...
#[pyfunction]
pub fn resize(py: Python<'_>, image: PyImage, new_size: (usize, usize), interpolation: &str) -> PyResult<PyImage> {
    let interpolation = match interpolation.to_lowercase().as_str() {
        "nearest" => InterpolationMode::Nearest,
        "bilinear" => InterpolationMode::Bilinear,
        _ => return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("Invalid interpolation mode")),
    };
    ...
    py.detach(|| resize_fast_rgb(&src, &mut dst, interpolation)).map_err(to_pyerr)?;
    Ok(out)
}
