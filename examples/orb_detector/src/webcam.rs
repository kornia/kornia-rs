...
pub fn webcam(args: &super::Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut webcam_orb_detector = OrbDectector::new(Default::default(), webcam_size)?;
    ...
    let mut img_orb_detector = OrbDectector::new(Default::default(), img_grayf32.size())?;
    img_orb_detector.detect(&img_grayf32)?;
    let img_extract = img_orb_detector.extract(&img_grayf32)?;
    ...
    loop {
        ...
        webcam_orb_detector.detect(&webcam_frame_grayf32)?;
        let webcam_extraction = webcam_orb_detector.extract(&webcam_frame_grayf32)?;
        ...
        for &(i1, i2) in matches.iter() {
            let kp1 = webcam_orb_detector.get_detection().keypoints[i1];
            let kp2 = img_orb_detector.get_detection().keypoints[i2];
        ...
        let keypoints1: Vec<[f32; 2]> = webcam_orb_detector
            .get_detection()
            .keypoints
            .iter()
            .map(|kp| [kp.1, kp.0])
            .collect();
        let keypoints2: Vec<[f32; 2]> = img_orb_detector
            .get_detection()
            .keypoints
            .iter()
